    ...
    extends JDialog
{
  ...
  public PopupDialog(final Vector data, Vector<String> columns, Statistic center, boolean hideFirstColumn)
  {
    ...
    JLabel label = new JLabel(sf.format(center.fromDate.getValue()) + "  至  " + sf.format(center.toDate.getValue()));
    label.setBorder(BorderFactory.createEmptyBorder(2, 0, 8, 0));
    pane.add(label, BorderLayout.NORTH);

    JScrollPane scrollpane = new JScrollPane(itemTable);
    scrollpane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
    itemTable.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "exit");
  ...
  class StatChart
  {
    public void chart(String type, Vector<Vector> data)
    {
      ...
      JFreeChart jfreechart = ChartFactory.createPieChart(type + "统计", piedataset, true, true, false);
      ...
      final JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(PopupDialog.this));
