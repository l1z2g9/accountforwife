...
public class TableUtility
{
  ...
  //  static String[] fields = {"type", "date", "item", "money", "remark", "color"};

  public void deleteData(final JTable table)
  {
    if (table.getSelectionModel().isSelectionEmpty())
    {
      JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(table), "请选择数据后再执行删除!");
      return;
    }
  ...
  public void makeTableCell(final JTable table)
  {
    ...
    table.getColumnModel().getColumn(4).setCellEditor(new MoneyCellEditor());
    table.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer()
    {
          ...
          int row, int column)
      {
        ...
        else
        {
          ...
          if (color != null)
          {
            setBackground(new Color(Integer.valueOf(color.toString())));
          }
        }
  ...
  public void tableChanged(TableModelEvent e, boolean changedDate)
  {
    ...
    if (TableModelEvent.INSERT == e.getType() || TableModelEvent.DELETE == e.getType() || changedDate || row == -1 ||
        column == 6)
    {
      return;
    }
    ...
    DBManager.update(id, Constance.getFieldColumns().get(column - 1), data.toString());
    DBManager.updateColor(model);
  }
